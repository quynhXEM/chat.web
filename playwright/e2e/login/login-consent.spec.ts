...
test.use({
    config: {
        // The only thing that we really *need* (otherwise SOC Connect refuses to load) is a default homeserver.
...
test.describe("Login", () => {
    test.describe("Password login", () => {
        ...
        test("Loads the welcome page by default; then logs in with an existing account and lands on the home screen", async ({
            ...
            await expect(page.getByRole("heading", { name: "Welcome to SOC Connect!" })).toBeVisible();
