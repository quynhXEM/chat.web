...
_If you're looking for information on how to set up Jitsi in your SOC Connect, see
[jitsi.md](./jitsi.md) instead._

These docs are for developers wondering how the different conference buttons work
within SOC Connect. If you're not a developer, you're probably looking for [jitsi.md](./jitsi.md).
...
Widgets have a `postMessage` API they can use to interact with SOC Connect, which also allows
SOC Connect to interact with them. Typically this is most used by the sticker picker (an
...
stuff to plug into a room. They are a separate service to SOC Connect and are thus iframed
...
SOC Connect. Widgets currently have a "pop out" button which opens them in a new tab and
...
## Jitsi widgets generated by SOC Connect itself

When the user clicks on the call buttons by the composer, the integration manager is
not involved in the slightest. Instead, SOC Connect itself generates a widget event, this time
...
The Jitsi widget created by SOC Connect uses a local `jitsi.html` wrapper (or one hosted by
`https://app.element.io` for desktop users or those on non-https domains) as the widget
`url`. The wrapper has some basic functionality for talking to SOC Connect to ensure the
...
## The Jitsi wrapper in SOC Connect

Whenever SOC Connect sees a Jitsi widget, it ditches the `url` and instead replaces it with
